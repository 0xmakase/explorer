...
export class LedgerWallet implements AbstractWallet {
  ...
  async sign712(tx: Transaction): Promise<Uint8Array> {
    ...
    const types = generateTypes(
      defaultMessageAdapter[tx.messages[0].typeUrl].getTypes()
    );
    ...
    const rawTx = this.makeRawTxEvmos(
      sender,
      tx.messages,
      tx.memo,
      tx.fee,
  ...
  makeRawTxEvmos(sender, messages, memo, fee, signature, chain): Uint8Array {
    ...
    // Create the txRaw
    const prototx = createTxRawEIP712(
      evmos.legacyAmino.body,
      evmos.legacyAmino.authInfo,
      extension
    );
